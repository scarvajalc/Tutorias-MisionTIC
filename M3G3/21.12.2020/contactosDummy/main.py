...
# Actualizar usando la base de datos dummy
@app.put("/contactos/actualizar/")
async def actualizar_contacto(contacto: db_dummy.Contacto):
    if db_dummy.obtener_contacto_por_id(contacto.id) is None:
        raise HTTPException(status_code=400, detail="Contacto inexistente")
    # db_dummy.contactos[contacto.id] = contacto
    db_dummy.contactos[contacto.id].direccion = contacto.direccion
    return {"mensaje": "Datos del contacto " + contacto.id + " actualizados correctamente"}


@app.put("/contactos/db/actualizar/")
async def actualizar_contacto_db(contacto: db_dummy.Contacto, db: Session = Depends(obtener_db)):
    if repositorio.obtener_contacto_por_id(db, contacto.id) is None:
        raise HTTPException(status_code=400, detail="Contacto inexistente")
    repositorio.actualizar_contacto(db, contacto)
    return {"mensaje": "Datos del contacto " + contacto.id + " actualizados correctamente"}


@ app.delete("/contactos/db/eliminar/{contacto_id}")
async def eliminar_contacto_db(contacto_id: str, db: Session = Depends(obtener_db)):
    if repositorio.obtener_contacto_por_id(db, contacto_id) is None:
        raise HTTPException(status_code=400, detail="Contacto inexistente")
    repositorio.eliminar_contacto(db, contacto_id)
    return {"mensaje": "Datos del contacto " + contacto_id + " eliminados correctamente"}
